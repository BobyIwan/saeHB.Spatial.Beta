...
#' @param tau.v Initial value for the random effect precision \eqn{(\tau_v)}. Default is \code{1}.
...
beta_lerouxcar <- function(formula, proxmat, data,
  ...
  inits_list <- lapply(1:chains, function(c) {
    list(v = rep(0, N), beta = mu_beta, tau_v = tau.v, rho = 0.5, phir = 1,
  ...
  params <- c("mu", "beta", "rho", "tau_v", "sigma2_v", "v", "phi")
  ...
  rownames(coefficient) <- c(paste0("beta[", seq_len(nvar) - 1, "]"), "rho", "phi")
