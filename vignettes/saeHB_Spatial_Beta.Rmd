...
a.  **SAR Model:** The random effects follow a Simultaneous Autoregressive process, $\mathbf{v} \sim N_m(\mathbf{0}, \mathbf{G})$, with the covariance dispersion matrix defined as:

    $$\mathbf{G} = \sigma_u^2 [(\mathbf{I} - \rho \mathbf{W})^T (\mathbf{I} - \rho \mathbf{W})]^{-1}$$

    Where $\rho$ is the spatial autocorrelation parameter and $\mathbf{W}$ is a row-standardized spatial weights matrix.
