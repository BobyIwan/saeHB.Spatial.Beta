test_that("Unit Testing for betaDeffNonSpatial: Execution and Output Format", {
  ...
  suppressWarnings({
    ...
    pdf(file = NULL)
    res_sampled <- betaDeffNonSpatial(
      formula = y ~ x1 + x2,
      DEFF = "deff",
      n_i = "n_i",
      data = dataBeta,
...
test_that("Unit Testing for betaDeffNonSpatial: Error Handling", {
  ...
  expect_error(
    betaDeffNonSpatial(y ~ x1 + x2, "deff", "n_i", data = data_invalid_y, plot = FALSE),
  ...
  expect_error(
    betaDeffNonSpatial(y ~ x1 + x2, "deff", "n_i", data = data_invalid_x, plot = FALSE),
