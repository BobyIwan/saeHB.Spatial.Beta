test_that("Unit Testing for betaDeffSAR: Execution and Output Format", {

  suppressWarnings({
    # Case 1: Fully sampled data
    pdf(file = NULL)
    res_sampled <- betaDeffSAR(
      formula = y ~ x1 + x2,
      DEFF = "deff",
      n_i = "n_i",
      proxmat = weight_mat,
...
test_that("Unit Testing for betaDeffSAR: Error Handling", {
  # Case 4: Response variable (y) not between 0 and 1
  data_invalid_y <- dataBeta
  data_invalid_y$y[5] <- 1.5
  expect_error(
    betaDeffSAR(y ~ x1 + x2, "deff", "n_i", weight_mat, data_invalid_y, plot = FALSE),
  ...
  expect_error(
    betaDeffSAR(y ~ x1 + x2, "deff", "n_i", weight_mat, data_invalid_x, plot = FALSE),
    "Auxiliary variables contain NA values"
  )

  # Case 6: Iteration update is less than 3
  expect_error(
    betaDeffSAR(y ~ x1 + x2, "deff", "n_i", weight_mat, dataBeta, iter.update = 2, plot = FALSE),
  ...
  expect_error(
    betaDeffSAR(y ~ x1 + x2, "deff", "n_i", wrong_W, dataBeta, plot = FALSE),
  ...
  na_W[1, 2] <- NA
  expect_error(
    betaDeffSAR(y ~ x1 + x2, "deff", "n_i", na_W, dataBeta, plot = FALSE),
    "Proximity matrix contains NA"
  )

  # Case 10: Formula without predictor
  expect_error(
    betaDeffSAR(y ~ 1, "deff", "n_i", weight_mat, dataBeta, plot = FALSE),
